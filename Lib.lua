...
shared.textService = game:GetService("TextService")
shared.inputService = game:GetService("UserInputService")
...
local library = {design = shared.design == "kali" and "kali" or "uwuware", tabs = {}, draggable = true, flags = {}, title = "uwuware", open = false, mousestate = shared.inputService.MouseIconEnabled, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "uw_configs", fileext = ".uw"}
...
function library:GetConfigs()
	...
	local files = {}
	local a = 0
	local ok2, listing = pcall(listfiles, self.foldername)
	if not ok2 then return {} end
	for i,v in next, listing do
...
library.createColorPickerWindow = function(option)
	...
	option.hexInput = option.hexBox:FindFirstChildOfClass("TextBox")
	...
	option.rgbInput.FocusLost:Connect(function()
		...
		local r2, g2, b2 = library.round(option.color)
		option.rgbInput.Text = table.concat({r2, g2, b2}, ",")
	end)
	...
	if option.hexInput then
		option.hexInput.FocusLost:Connect(function()
			...
			local r2, g2, b2 = library.round(option.color)
			option.hexInput.Text = string.format("#%02x%02x%02x", r2, g2, b2)
		end)
	end
	...
	function option:updateVisuals(Color)
		...
		if option.trans and transMain then
...
function library:Close()
	...
	if self.main then
		...
		if self.cursor1 then self.cursor1.Visible = self.open end
	end
end
...
function library:Init()
	...
	-- Potassium-compatible GUI parenting
	pcall(function()
	...
	-- Drawing cursor (wrapped in pcall for safety)
	...
	-- Config list auto-updater
	task.spawn(function()
		while library do
			task.wait(1)
			local Configs = self:GetConfigs()
			if library and library.options["Config List"] then
