...
LPH_JIT_MAX(function() -- Main Cheat
    ...
    local function getClosest(origin, fov, deadfov, visibleCheck, partName)
        ...
        local targetPartNames = partName == "Closest" and {"Head", "Torso"} or {partName or "Head"}
        ...
        replicationInterface.operateOnAllEntries(function(player, entry)
            ...
            if character and entry._isEnemy then
                ...
                for _, candidatePartName in ipairs(targetPartNames) do
                    local bodyPart = character[candidatePartName]

                    if bodyPart then
                        local target = bodyPart.Position
                        ...
                        if not visibleCheck or not raycast(localposition, target - localposition, physicsignore) then
                            ...
                            local screenDirection = screenPosition.Z
    ...
    -- Hoisted here so network:send's spawn handler can reference them
    local backtrackQueues   = {}
    local backtrackNextSnap = {}
    local forwardtrackGhosts   = {}
    local forwardtrackLastPos  = {}
    local forwardtrackLastTime = {}
    local forwardtrackVel      = {}
    local forwardtrackSnapNext = {}
    ...
    function network:send(name, ...)
        ...
        elseif name == "newbullets" then
            ...
            if wapus:GetValue("Silent Aim", "Enabled") and (wapus:GetValue("Silent Aim", "Hit Chance") >= chanceOne) then
                local target, entry, part = getClosest(silentaimfov.Position, wapus:GetValue("Silent Aim", "Use FOV") and silentaimfov.Radius, wapus:GetValue("Silent Aim", "Use Dead FOV") and silentaimdeadfov.Radius, wapus:GetValue("Silent Aim", "Visible Check"), wapus:GetValue("Silent Aim", "Target Part"))
...
--[[ synz has upval instability
    ...
    -- ── Enemy Chams via Highlight (Adornee) ────────────────────────────────────
    -- Uses Roblox Highlight instances with Adornee set to the enemy character.
    -- Two color modes via dropdown: "Highlight" (fill+outline) or "Chams" (cham.new).
    -- Both switch color based on whether the enemy is visible from camera.
    local enemyHighlights  = {}   -- [player] = Highlight instance (Adornee mode)
    local enemyChamProps   = {}   -- [player] = cham.new properties table
    local enemyChamUncache = {}   -- [player] = cham.new uncache fn

    local chamRayParams = RaycastParams.new()
    chamRayParams.FilterType = Enum.RaycastFilterType.Exclude

    local function destroyEnemyChams()
        for player, h in pairs(enemyHighlights) do
            pcall(function() h:Destroy() end)
            enemyHighlights[player] = nil
        end
        for player, u in pairs(enemyChamUncache) do
            pcall(u)
    ...
    local function rebuildEnemyChams()
        destroyEnemyChams()
        if not wapus:GetValue("Enemy ESP%%Highlight Chams") then return end

        local method = wapus:GetValue("Enemy ESP%%Chams Method") or "Highlight"
    ...
    -- Visibility-switching loop: update colors each RenderStepped
    table.insert(connectionList, game:GetService("RunService").RenderStepped:Connect(function()
        if not wapus:GetValue("Enemy ESP%%Highlight Chams") then return end
        ...
        replicationInterface.operateOnAllEntries(function(player, entry)
            ...
            if method == "Highlight" then
                local h = enemyHighlights[player]
                -- Rebuild if Highlight was destroyed or character changed
                ...
                -- Rebuild if cham was lost or character changed
                if not props then
    ...
    table.insert(connectionList, runService.Heartbeat:Connect(function()
        ...
        replicationInterface.operateOnAllEntries(function(player, entry)
            ...
            -- ── Velocity: smoothed _receivedPosition delta ──
            ...
            if curPos then
                ...
                if prevPos and prevTime then
                    ...
                    if dt > 0.008 then  -- minimum 8ms between samples to avoid noise
                        local rawVel = (curPos - prevPos) / dt
                        -- clamp absurd velocities (teleports, rubber-banding)
                        if rawVel.Magnitude < 200 then
                            local prev = forwardtrackVel[player] or rawVel
                            -- 0.2 lerp: smooth but still tracks direction changes
                            forwardtrackVel[player] = prev:Lerp(rawVel, 0.2)
                        end
...
LPH_NO_VIRTUALIZE(function() -- Make UI
    ...
    silentaim:AddDropdown("Target Part", "Head", {"Head", "Torso", "Closest"}, getCallback("Silent Aim%%Target Part"))
    ...
    enemyesp:AddDropdown("Chams Method", "Highlight", {"Highlight", "Chams"}, getCallback("Enemy ESP%%Chams Method"))
